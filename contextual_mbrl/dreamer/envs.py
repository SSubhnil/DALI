...
# Benchmark-aligned discrete context values for DRAMA comparison.
# Values from CausalWorldModel/configs/baseline_alignment/dali_factor_mapping.yaml
_BENCHMARK_CONTEXTS = {
    "dmc_walker": {
        "gravity": {
            "train":         [3.924, 5.886, 9.810, 14.715, 19.620],
            "test_iid":      [4.905, 7.848, 12.263, 17.168],
            "test_ood_mild": [2.453, 2.943, 24.525, 29.430],
            "test_ood_hard": [1.472, 1.962, 34.335, 39.240],
        },
        "actuator_strength": {
            "train":         [0.30, 0.50, 1.00, 2.00, 3.00],
            "test_iid":      [0.40, 0.75, 1.50, 2.50],
            "test_ood_mild": [0.15, 0.20, 4.00, 5.00],
            "test_ood_hard": [0.10, 0.12, 5.50, 6.00],
        },
    },
    "dmc_quadruped": {
        "gravity": {
...
def make_envs(config, **overrides):
    ...
    for index in range(config.envs.amount):
        ...
        if config.envs.restart:
            ctor = bind(embodied.wrappers.RestartOnException, ctor, maxfails=50, window=600)
        ctors.append(ctor)
...
def make_env(config, **overrides):
    ...
    if suite == "carl":
        ...
        (lambda m: bool(m) and tuple(m) != (-1,))(getattr(config.env.atari, "modes", None))
...
def make_atari_context_env(config, **overrides):
    ...
    _modes = getattr(config.env.atari, "modes", None)
    modes = None if (not _modes or tuple(_modes) == (-1,)) else _modes
    _diffs = getattr(config.env.atari, "difficulties", None)
    difficulties = None if (not _diffs or tuple(_diffs) == (-1,)) else _diffs
    ...
    if getattr(config.env.carl, "regime", "A") == "B":
        ...
        if task == "dmc_quadruped":
            if regime_b_factors == "K3":
                from benchmark.wrappers.carl_intra_episode import make_quadruped_regime_b_K3_wrapper
            ...
            if regime_b_factors == "K3":
                from benchmark.wrappers.carl_intra_episode import make_walker_regime_b_K3_wrapper
        ...
        if regime_b_factors == "K3_mixed":
            from benchmark.wrappers.carl_reward_switch import CARLRewardModeSwitcher
            reward_modes = list(getattr(config.env.carl, "regime_b_reward_modes",
